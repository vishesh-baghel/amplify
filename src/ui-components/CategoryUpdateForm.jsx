...
export default function CategoryUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = categoryRecord
      ? { ...initialValues, ...categoryRecord, Products: linkedProducts }
      : initialValues;
  ...
  React.useEffect(resetStateValues, [categoryRecord, linkedProducts]);
        ...
        try {
          ...
          productsToUnLink.forEach((original) => {
            if (!canUnlinkProducts) {
              throw Error(
                `Product ${original.id} cannot be unlinked from Category because categoryID is a required field.`
              );
            }
            promises.push(
              DataStore.save(
